...
Task 10 currently has two completed local implementation slices and a third in progress. The shared domain now validates evidence provenance, distinct current and target seniority, inactive evidence-backed suggestions, flexible onboarding signals, and named searches that may begin from a role, industry/domain, skill, responsibility, or keyword without allowing an empty search; 150 domain tests and domain typechecking pass. Migration 7 adds six forced-RLS career-data tables, a private owner-path CV bucket capped at 5 MiB, atomic one-current-CV replacement, column-limited evidence review, owner-only suggestion decisions, bounded extraction proposals and error codes, and no administrator override policy. The static seven-migration/seventeen-table verifier, guardrails, and its focused tests pass. A 37-assertion pgTAP specification covers pending/owner/other-user/administrator access, forged CV evidence, service extraction, CV replacement, suggestion acceptance, and cascading deletion; real execution remains blocked by the same Docker/Supabase pre-live gate. The new profile package rejects DOCM/legacy/unknown formats, mismatched extension/MIME/magic, unsafe names, empty files, and files over 5 MiB before parsing. Its deterministic fallback emits only explicit, offset-backed concepts and inactive evidence-backed role suggestions, redacts contact patterns from excerpts, and returns no raw CV text; 23 fictional tests pass. DOCX/PDF parser work is still open because the reviewed libraries are absent and registry access is blocked. Real CV upload remains disabled.
