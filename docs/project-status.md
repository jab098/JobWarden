...
| 10. Career profile, onboarding, and CV extraction                      | active   | Contracts, private data, file gate, and fallback proposal built |
...
Task 10 now has three completed local implementation slices; the authenticated extraction runtime is next. The shared domain validates evidence provenance, distinct current and target seniority, inactive evidence-backed suggestions, flexible onboarding signals, and named searches that may begin from a role, industry/domain, skill, responsibility, or keyword without allowing an empty search; 150 domain tests and domain typechecking pass. Migration 7 adds six forced-RLS career-data tables, a private owner-path CV bucket capped at 5 MiB, atomic one-current-CV replacement, column-limited evidence review, owner-only suggestion decisions, bounded extraction proposals and error codes, and no administrator override policy. The static seven-migration/seventeen-table verifier, guardrails, and its focused tests pass. A 37-assertion pgTAP specification covers pending/owner/other-user/administrator access, forged CV evidence, service extraction, CV replacement, suggestion acceptance, and cascading deletion; real execution remains blocked by the same Docker/Supabase pre-live gate. The profile package rejects DOCM/legacy/unknown formats, mismatched extension/MIME/magic, unsafe names, empty files, and files over 5 MiB before parsing. DOCX extraction rejects unsafe or duplicate paths, excessive entries or declared expansion, external relationships, foreign-namespace text, executable parts, and active XML before selected OOXML is inflated; PDF extraction rejects encrypted or malformed files and documents above 250 pages before sequential text extraction. Both formats share the 100,000-character and 20-second ceilings. The deterministic fallback emits only explicit, offset-backed concepts and inactive evidence-backed role suggestions, redacts contact patterns from excerpts, and returns no raw CV text; 44 fictional profile-package tests pass. Real CV upload remains disabled.
