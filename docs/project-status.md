...
| 10. Career profile, onboarding, and CV extraction                      | active   | Local implementation complete; review/publication still pending |
...
Task 10's local implementation is complete on `codex/task-10-career-profile`; independent PR review, publication, merge, and merge-commit verification remain pending and Task 11 has not started. The shared domain validates evidence provenance, distinct current and target seniority, inactive evidence-backed suggestions, flexible onboarding signals, and named searches that may begin from a role, industry/domain, skill, responsibility, or keyword without allowing an empty search. Ten migrations now provide forced-RLS owner data, a private owner-path CV bucket capped at 5 MiB, one-current-CV replacement with failed-extraction rollback, storage-first inactive cleanup, owner-derived evidence/suggestion review, bounded raw proposals with hourly 24-hour expiry, and an auditable application-wide AI ceiling. The authenticated Edge Function is deterministic-first, idempotent, limited to one concurrent extraction per user, uses only sanitised logs, and treats optional schema-validated Workers AI output as discardable; its daily allowance defaults to zero. The repository and editorial `/profile` workflow support explicit career direction, current/target seniority, user evidence, extracted-evidence confirm/exclude, suggestion decisions, named searches, CV deletion, and full-profile deletion. The local preview is immutable and fictional, and there is no file input. Safe DOCX/PDF extraction retains the 5 MiB, 2,000-entry, 250-page, 100,000-character, and 20-second ceilings. Desktop and true 390 px browser checks passed the profile flow, navigation, focus, no-overflow, and no-error-overlay checks. The permanent operations guide and personal-data inventory define replacement, cleanup, expiry, AI, deletion, incident, and activation ordering. Real CV upload is independently disabled in the web and by a database setting that blocks Storage writes, registration, and extraction claims; both remain off until live approved authentication, private Storage/RLS/deletion exercises, and Docker-backed database reset/pgTAP pass.
