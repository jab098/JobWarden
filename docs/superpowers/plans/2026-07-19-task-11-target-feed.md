...
- Seniority (15): detect job seniority markers deterministically from title (word list per level, e.g. "graduate|junior" → junior … "head|director|vp" → director). No marker → award 10 (unknown neutral, not negative and not full). `targetSeniority` of `unspecified` → award 10 regardless of marker (a don't-care preference is never a negative inference). Marker equals `targetSeniority` → 15; adjacent level → 8; otherwise → 0.
