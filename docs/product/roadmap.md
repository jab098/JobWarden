...
| 30b  | Provider vocabulary widening                                               | shipped  | None; `lever` accepted. Ashby joined in Task 31; Workable stays rejected until 32     |
...
5. ~~**Task 31 — Ashby adapter.**~~ **Done, 2026-07-21.** The adapter, the provider value, and the compliance record all landed together, and the source ships disabled. The vocabulary migration was **generated rather than transcribed**, as `docs/project-status.md` requires: each live definition was extracted with `pg_get_functiondef`, the provider list substituted, and the result diffed against the extraction to prove exactly seven changed lines across six functions. **Task 32 — Workable adapter** is next and is the same shape; it will hit the barrel-export name collision Task 31 hit, so prefix its exported helpers by provider from the start. Confirm the Workable endpoint against the provider's own documentation at slice start; do not carry one forward from this file.
