...
- The 55-second lifecycle deadline starts before request-body parsing and covers request streaming, claim, Storage download, parsing, optional AI, lease renewal, and finalisation. Every body read races that deadline; an abort cancels a stalled reader and returns only the sanitised `extraction_timeout` code. The stream is also cancelled beyond 2,048 bytes when length is absent, and truncated extraction fails visibly instead of persisting an incomplete proposal.
- DOCX parsing rejects markup-compatibility alternate-content branches instead of combining `Choice` and `Fallback` evidence. It resolves declared `mc:Ignorable` prefixes to namespace URIs, suppresses the full ignorable extension subtree even when it contains WordprocessingML descendants, and fails closed on undeclared or malformed prefixes and unsupported compatibility instructions. A bounded PDF lexical preflight decodes and rejects exact structural name tokens for catalog associated files, XFA form data, unsupported actions, and object streams while skipping valid comments, balanced literal strings, hexadecimal strings, and validated stream payloads. A stream body is skipped only when one direct non-negative integer `/Length`, exact end-of-line delimiters, and the matching `endstream` establish its extent; indirect, missing, duplicate, malformed, or mismatched lengths fail closed, and container nesting beyond a fixed depth is rejected. Benign visible `/AF` and `/XFA` text and normal appearance names such as `/AP` and `/AS` remain accepted. PDF.js then rejects parser-reported JavaScript, open actions, attachments, page actions, and annotations—including unsupported rich-media fallbacks—plus invisible text modes, zero alpha, soft masks, and transparent fill/stroke operators before text becomes evidence. The display optional-content configuration is fetched under the same extraction deadline; marked-content OCG references must resolve and be visible, while catalog-disabled groups, OCMD/unknown references, and malformed or unbalanced scopes fail closed.
