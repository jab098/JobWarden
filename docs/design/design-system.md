...
- **A surface that measures the user's own activity has a first state, and it is not a grid of zeros.** Home branches on whether the account has done anything at all (tracked applications and decisions, never matches, which arrive on their own): before that it leads with what JobWarden has already found, then the actions that start the measuring, and it drops the window switcher because nothing is being measured yet. Afterwards, any single panel with nothing in it says what it will show and links to the action that fills it, rather than drawing an empty chart. Zero is a real and honest value; a page made only of zeros reports a beginning as a failure.
