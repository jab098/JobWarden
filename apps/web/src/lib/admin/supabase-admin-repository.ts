...
const sourceHealthRowSchema = z.object({
  ...
  last_successful_sync_at: timestampSchema.nullable(),
  latest_run_status: z.enum(["running", "succeeded", "failed"]).nullable(),
  latest_error_code: z.string().min(3).max(100).nullable(),
  ...
  temporary_roles: z.number().int().nonnegative(),
  full_time_roles: z.number().int().nonnegative(),
...
const runColumns = [
  ...
  "status",
  "response_complete",
  "excluded_non_uk_count",
  "quarantined_ambiguous_count",
  "quarantined_invalid_url_count",
  "unrecognised_locations",
  "received_count",
  "eligible_count",
  "upserted_count",
  "unchanged_count",
    ...
    async listSourceHealth() {
      try {
        ...
        return rows.map<SourceHealthView>((row) => ({
          ...
          temporaryRoles: row.temporary_roles,
          fullTimeRoles: row.full_time_roles,
    ...
    async listIngestionRuns(limit) {
      try {
        ...
        return rows.map<IngestionRunView>((row) => ({
          ...
          excludedNonUkCount: row.excluded_non_uk_count,
          quarantinedAmbiguousCount: row.quarantined_ambiguous_count,
          quarantinedInvalidUrlCount: row.quarantined_invalid_url_count,
          unrecognisedLocations: row.unrecognised_locations,
