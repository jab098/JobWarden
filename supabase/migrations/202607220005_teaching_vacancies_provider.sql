-- Task 38 — Teaching Vacancies joins the provider vocabulary.
--
-- The Department for Education's Teaching Vacancies service, read under Open
-- Government Licence v3.0 with no credential. Its dated compliance record is in
-- docs/product/source-coverage.md.
--
-- Like Reed and unlike the employer ATS boards, this is a single national
-- discovery source with a database-pinned identity rather than one row per
-- employer, so the constraint pins its board token, employer name and allowed
-- host, and `upsert_job_source` continues to refuse it: it is not an
-- administrator-configurable employer board.
--
-- Coverage is incremental. A bounded read of a paginated service is not a
-- snapshot, so an advert absent from what was read must never advance an
-- omission counter and can never close a job.
--
-- A six-hour floor matches Reed's. The service publishes no rate limit, so this
-- is politeness towards a free public service rather than a stated requirement.
--
-- Every function below is create-or-replace, never drop-and-create, because a
-- drop resets the ACL to EXECUTE for PUBLIC (see Task 25c). Bodies are
-- extracted verbatim from 202607220004, which last defined them; the only edit
-- is the provider vocabulary.
...
alter table public.job_sources
  ...
  add constraint job_sources_teaching_vacancies_minimum_interval check (
    provider <> 'teaching_vacancies' or minimum_sync_interval >= interval '6 hours'
  );

-- public.start_source_ingestion — from 202607220004_provider_vocabulary.sql
...
begin
  ...
  if not source_record.enabled
    or source_record.provider not in ('greenhouse', 'lever', 'reed', 'teaching_vacancies') then
...
-- public.enqueue_scheduled_ingestion — from 202607220004_provider_vocabulary.sql
...
begin
  ...
  where source.enabled
    and source.provider in ('greenhouse', 'lever', 'reed', 'teaching_vacancies')
    and (
...
-- public.claim_ingestion_requests — from 202607220004_provider_vocabulary.sql
...
-- public.upsert_ingested_jobs — from 202607220004_provider_vocabulary.sql
...
begin
  ...
  if not source_enabled or source_provider not in ('greenhouse', 'lever', 'reed', 'teaching_vacancies') then
    raise exception using errcode = '22023', message = 'source is not enabled for ingestion';
  end if;
