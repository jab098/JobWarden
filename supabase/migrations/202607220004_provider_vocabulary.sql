...
--
-- Adds lever, ashby and workable to the provider vocabulary. Lever's adapter
-- shipped in Task 30a but 'lever' was not a value the database would accept,
-- so the adapter could not be configured as a source. Ashby and Workable are
-- added now so Tasks 31 and 32 add an adapter and fixtures without reopening
-- security-definer SQL.
--
-- The three new boards are per-employer applicant-tracking systems, so they
-- are coverage_mode 'complete' like Greenhouse. Reed is unchanged: still
-- 'incremental', still pinned to its single gb-discovery identity, still held
-- to a six-hour minimum interval.
--
-- Every function is create-or-replace, never drop-and-create. Task 25c's
-- review found that a drop resets the ACL to PostgreSQL's default EXECUTE to
-- PUBLIC, which left a security-definer ingestion function reachable by the
-- anon key while the static verifier certified it as revoked. No signature
-- changes here, so no grant is added or revoked.
--
-- The function bodies below are extracted verbatim from the migrations that
-- last defined them; the only edits are the provider vocabulary itself.
...
-- public.upsert_job_source — from 202607180001_admin_operations.sql lines 8-159
...
begin
  ...
  if provider_name not in ('greenhouse', 'lever') then
...
-- public.start_source_ingestion — from 202607180003_uk_coverage_compensation.sql lines 260-307
...
begin
  ...
  if not source_record.enabled
    or source_record.provider not in ('greenhouse', 'lever', 'reed') then
...
-- public.enqueue_scheduled_ingestion — from 202607180003_uk_coverage_compensation.sql lines 309-344
...
begin
  ...
  where source.enabled
    and source.provider in ('greenhouse', 'lever', 'reed')
    and (
...
-- public.claim_ingestion_requests — from 202607180003_uk_coverage_compensation.sql lines 346-506
...
-- public.upsert_ingested_jobs — from 202607180003_uk_coverage_compensation.sql lines 508-733
...
begin
  ...
  if not source_enabled or source_provider not in ('greenhouse', 'lever', 'reed') then
    raise exception using errcode = '22023', message = 'source is not enabled for ingestion';
  end if;
...
-- private.rematerialize_canonical_job — from 202607200003_job_locations_writer.sql lines 24-147
...
begin
  ...
  order by
    ...
    case when source.provider in ('greenhouse', 'lever') then 0 else 1 end,
