-- Task 31. Add `ashby` to the provider vocabulary, alongside its adapter.
--
-- Task 30b added `lever` and deliberately left `ashby` and `workable` rejected,
-- because a provider value with no adapter lets an administrator configure a
-- source that saves, enables, and then fails at dispatch. This migration flips
-- `ashby` because `packages/ingestion/src/ashby.ts` ships in the same change.
-- `workable` stays rejected until Task 32.
--
-- **The function bodies below were generated, not transcribed.** Each live
-- definition was extracted with `pg_get_functiondef`, the provider list was
-- substituted, and the result was diffed against the extraction to prove that
-- exactly seven lines changed across six functions and that each changed line
-- adds only `'ashby'`. Hand-copying ~720 lines of definer SQL is how a subtle
-- hole gets in; `docs/project-status.md` records that instruction and this
-- migration follows it.
--
-- The six objects were confirmed against the live database at slice start
-- rather than read from the Task 30b plan, by selecting every function whose
-- `pg_get_functiondef` contains a provider literal. That returned exactly these
...
-- certifies it as revoked. The constraint is different — a check constraint
-- cannot be replaced in place — so it is dropped and re-added inside this same
-- transaction. Constraints carry no ACL, so the trap does not apply there.
--
-- Ashby is a per-employer board like Greenhouse and Lever: one documented
-- public request returns the whole board, so `coverage_mode` is `complete` and
-- the two-consecutive-omissions closure rule applies. It takes no
-- provider-specific minimum interval, following the Greenhouse and Lever
-- precedent — the general 15-minute floor in
-- `job_sources_minimum_sync_interval_check` applies. Reed and Teaching
-- Vacancies keep their 6-hour floors because they are national discovery
-- sources read repeatedly, which is a different shape.
--
-- `ashby` joins `upsert_job_source`'s accepted list because, unlike Reed and
-- Teaching Vacancies, it is a per-employer board and therefore administrator
-- configurable through the source form.
...
-- public.upsert_job_source — regenerated from the live definition with 'ashby' added.
...
begin
  ...
  if provider_name not in ('greenhouse', 'lever', 'ashby') then
...
-- public.start_source_ingestion — regenerated from the live definition with 'ashby' added.
...
begin
  ...
  if not source_record.enabled
    or source_record.provider not in ('greenhouse', 'lever', 'ashby', 'reed', 'teaching_vacancies') then
...
-- public.enqueue_scheduled_ingestion — regenerated from the live definition with 'ashby' added.
...
begin
  ...
  where source.enabled
    and source.provider in ('greenhouse', 'lever', 'ashby', 'reed', 'teaching_vacancies')
    and (
...
-- public.claim_ingestion_requests — regenerated from the live definition with 'ashby' added.
...
-- public.upsert_ingested_jobs — regenerated from the live definition with 'ashby' added.
...
begin
  ...
  if not source_enabled or source_provider not in ('greenhouse', 'lever', 'ashby', 'reed', 'teaching_vacancies') then
    raise exception using errcode = '22023', message = 'source is not enabled for ingestion';
  end if;
...
-- private.rematerialize_canonical_job — regenerated from the live definition with 'ashby' added.
...
begin
  ...
  order by
    ...
    case when source.provider in ('greenhouse', 'lever', 'ashby') then 0 else 1 end,
