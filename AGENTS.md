...
Before UI work, read `docs/design/ui-direction.md`, `docs/design/design-system.md`, and `docs/standards/frontend-traps.md`. The traps file records implementations that typecheck, pass tests, and silently do nothing on this exact stack; several design-system rules exist only because of them. When verifying UI, assert the rendered result, never that a class was applied or a rule exists. When available, load `anthropic-skills:web-artifacts-builder` for its anti-slop design guidance, `vercel:shadcn` for component composition, and `vercel:react-best-practices` for the final TSX review. The approved product specification and JobWarden UI direction override generic skill defaults.
